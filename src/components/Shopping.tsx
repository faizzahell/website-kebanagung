...
import { type Product, productsData } from '../constants/product';
              ...
              <div className="p-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-3xl font-bold text-green-700">{formatPrice(selectedProduct.price)}</span>
                    {selectedProduct.originalPrice && (
                      <span className="text-xl text-slate-400 line-through">{formatPrice(selectedProduct.originalPrice)}</span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3 pt-4">
                    <button className="flex-1 bg-green-700 text-white px-6 py-3 rounded-lg font-medium hover:bg-green-800 transition-colors">
                      Hubungi Penjual
                    </button>
                    <button className="px-4 py-3 border border-green-700 text-green-700 rounded-lg hover:bg-green-50 transition-colors">
...
const ProductCard: React.FC<ProductCardProps & { onOpenModal: (product: Product) => void }> = ({ product, onOpenModal }) => {
  const formatPrice = (price: number): string => {
    return new Intl.NumberFormat('id-ID', {
      style: 'currency',
      currency: 'IDR',
